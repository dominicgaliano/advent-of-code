import sys
import re
from functools import reduce


class Solution:

    def __init__(self):
        self.data = []

    def loadData(self, filePath: str) -> None:
        with open(filePath, "r", encoding="utf-8") as file:
            self.data = list(file)

    def getWinningNumbers(self, game: str) -> list:
        return set(map(int, re.findall(r'(\d+)(?![^\r\n:]*\:)(?=[^\r\n]*\|)', game)))

    def getCardNumbers(self, game: str) -> set:
        return list(map(int, re.findall(r'(\d+)(?![^\r\n|]*\|)', game)))

    def getCardMatches(self, game: str) -> int:
        winningNumbers = self.getWinningNumbers(game)
        cardNumbers = self.getCardNumbers(game)

        def reducer(points, cardNumber):
            if cardNumber in winningNumbers:
                return points + 1
            return points

        return reduce(reducer, cardNumbers, 0)

    def getSolution(self) -> int:
        cardCounts = [1 for card in self.data]
        maxCardCount = len(cardCounts) - 1

        for cardNumber, line in enumerate(self.data):
            numMatches = self.getCardMatches(line)

            for i in range(1, numMatches+1):
                if (i + cardNumber) > maxCardCount:
                    break
                cardCounts[i+cardNumber] += cardCounts[cardNumber]

        return sum(cardCounts)


def main() -> None:

    if len(sys.argv) != 2:
        print("Usage: python3 solution2.py <file_name>")
        exit(1)

    solution = Solution()
    solution.loadData(sys.argv[1])
    print(solution.getSolution())


if __name__ == "__main__":
    main()
